...
use super::{
    error::FatalConnectionError,
    nc_loop::user_event::{PresenceData, TextMessageData, UserEvent},
};
use super::{
    error::{ConnectionError, NonFatalConnectionError},
    nc_loop::nats_message::NatsMessage,
};
use crate::db::{
...
impl WsLoop {
    pub async fn handle(
        ...
        'ws_loop: while let Some(message) = tokio::select! {
            ...
            match message {
                Message::Text(message) => match UserOperation::from_str(&message) {
                    Ok(user_operation) => {
        ...
        match user_operation {
            ...
            UserOperation::Messages {
                ...
                if conversation_id.get_role_of_username(&self.username)
                    == ConversationRole::NotInConversation
                {
                    let _ = err_tx.send(ConnectionError::Fatal(FatalConnectionError::Forbidden(
                        "User attempted to get messages of conversation not belonging to",
                    )));
                    return;
                }
